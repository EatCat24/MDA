...
aws_access_key_id = 'AKIAYS2NTEGRHNGVP666'
aws_secret_access_key = '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'
...
bucket_name = 'xwynews' # Set to my S3 bucket name
...
# Update file paths, now using S3 keys
file_names = [
...
# Process 'EventType and EventLevel' columns in the second DataFrame
df2 = dfs[1]
...
# Extract rows with 'event type trip' starting with 'P039' or 'P003'
filtered_dfs = [df[df['event type trip'].str.startswith(('P039', 'P003'), na=False)] for df in dfs]

# Extract four columns and merge
...
# Data for trend chart of cardiac arrest numbers
...
# Trend chart of cardiac arrest numbers
...
def plot_evaluation_metrics():
    try:
        ...
        X_train, X_test = train_test_split(X_res, test_size=0.3, random_state=42)
        y_train, y_test = train_test_split(y_res, test_size=0.3, random_state=42)
...
def update_content(chart_type):
    ...
    # Call the appropriate plotting function based on chart_type
    fig = None
